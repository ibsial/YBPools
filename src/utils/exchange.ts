...
const withdrawNetworks: {[key: string]: {name: string; token: string; fee: string}} = {
    Optimism: {
        name: 'Optimism',
        token: 'ETH',
        fee: '0.00004'
...
class Okx {
    ...
    async withraw(
        ...
        try {
            ...
            console.log(`current exchange balance: ${c.underline(balance)} ${currency} want to withdraw: ${c.underline(val)} ${currency}`)
            let i = 0
            ...
            while (i < max) {
                ...
                if (i != 0) {
                    ...
                    //         val.toString()
                    //     )} ${currency} | ${((((i + 1) * 5) / (max * 5)) * 100).toFixed(0)}%`
                    // )
                }
    ...
    async transferToMain(subAccounts: {name: any; balance: any}[], currency = 'ETH') {
        let res
        for (let acc of subAccounts) {
            ...
            log(c.green(`\nfound ${res.amount} ETH on subacc ${acc.name}`))
        }
        return res
    }
}
