...
// import {getRandomProxyProvider} from '../utils'
...
async function waitBalance(
    ...
    while (currentBalance <= balanceBefore) {
        ...
        await defaultSleep(checkPeriod, false)
        waitTime += checkPeriod
    }
    // console.log(` --> received ${formatEther(currentBalance - balanceBefore)}\n`)
    return true
}
...
async function sendTx(
    ...
    waitConfirmation = true,
    retryParams = {maxRetryCount: 3, retryInterval: 3, needLog: false}
) {
