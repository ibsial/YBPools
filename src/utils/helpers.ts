        ...
        try {
            ...
            if (needLog) {
                if (errorMessage == '') {
                    ...
                    console.log(`retry log [${i}]:`, errorMessage)
                }
            }
...
class Random {
    ...
    unusedEmails: {email: string; password: string; refreshToken: string; clientId: string; authToken?: undefined}[] | undefined
    unusedOkxs: {okxUid: string; depositAddress: string}[] | undefined
    ...
    getRandomProxy() {
        return this.proxies == undefined ? undefined : this.getRandomElementFromArray(this.proxies)
    }
}
...
async function importEmails(path: string) {
    let data: string[] = []
    let instream = fs.createReadStream(path)
    let rl = readline.createInterface(instream)
    rl.on('line', (line) => {
    ...
    let element = '----'
    for (let str of data) {
        let splittedLine = str.split(element)
        emails.push({email: splittedLine[0], password: splittedLine[1], refreshToken: splittedLine[3], clientId: splittedLine[2]})
    }
    return emails
}
async function importOkxData(path: string) {
    let data: string[] = []
    let instream = fs.createReadStream(path)
    let rl = readline.createInterface(instream)
    rl.on('line', (line) => {
    ...
    for (let str of data) {
        let splittedLine = str.split(element)
        OKXs.push({okxUid: splittedLine[0],depositAddress: splittedLine[1]})
    }
    return OKXs
}
